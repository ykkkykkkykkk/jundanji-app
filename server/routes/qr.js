...
router.post('/flyers/:flyerId/qr/generate', authMiddleware, async (req, res) => {
  ...
  const flyer = await db.prepare('SELECT id, qr_code FROM flyers WHERE id = ?').get(flyerId)
  if (!flyer) {
    return res.status(404).json({ ok: false, message: '전단지를 찾을 수 없습니다.' })
  }

  const qrCode = crypto.randomUUID()
  await db.prepare('UPDATE flyers SET qr_code = ? WHERE id = ?').run(qrCode, flyerId)
...
router.get('/flyers/:flyerId/qr', async (req, res) => {
  const { flyerId } = req.params

  const flyer = await db.prepare('SELECT id, store_name, qr_code, qr_point FROM flyers WHERE id = ?').get(flyerId)
  if (!flyer) {
    return res.status(404).json({ ok: false, message: '전단지를 찾을 수 없습니다.' })
  }
...
router.post('/qr/verify', async (req, res) => {
  ...
  const user = await db.prepare('SELECT id, points, role FROM users WHERE id = ?').get(userId)
  ...
  const flyer = await db.prepare('SELECT id, store_name, title, qr_point FROM flyers WHERE qr_code = ?').get(qrCode)
  if (!flyer) {
